...
public class VillagerEntry : MonoBehaviour
{
    ...
    private void SetupVillagerInfo()
    {
        ...
        var professionManager = villagerReference.GetComponent<VillagerProfession>();
        if (professionManager != null)
        {
            ProfessionType = professionManager.ProfessionData.professionName;
            professionText.text = $"Profession: {ProfessionType}";

            // Get and set current state
            var context = villagerReference.GetComponent<VillagerBrain>();
